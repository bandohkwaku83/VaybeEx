const OTP_STORAGE_PREFIX = "trripx-organizer-otp:";
const OTP_TTL_MS = 10 * 60 * 1000;
const DEMO_OTP = "123456";

export function sendOrganizerOtp(email: string): string {
  const code = DEMO_OTP;
...
export function verifyOrganizerOtp(email: string, code: string): boolean {
  try {
    const raw = sessionStorage.getItem(`${OTP_STORAGE_PREFIX}${email.toLowerCase()}`);
    if (!raw) return code === DEMO_OTP;
...
export function clearOrganizerOtp(email: string): void {
